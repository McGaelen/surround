...
public class SurroundPanel extends JPanel{
	...
	private void setUpButtonsAndGame() {
		...
		int minefield = JOptionPane.showConfirmDialog(null, "Play in MineField mode?");
		if (minefield == JOptionPane.YES_OPTION) {
			game.minefield();
		} 
		else if (minefield == JOptionPane.CANCEL_OPTION) {
			System.exit(0);
		}
	...
	private void setUpLabels() {
		...
		for (int i = 0; i < playerWins.length; i++) {
			playerWins[i] = new JLabel("P" + (i+1) + " Wins: " + winsTracker[i] + "  ");
		...
		while (validSize == false) {
			try {
				...
				while (bdSize < 3 || bdSize > 20) {
					JOptionPane.showMessageDialog(null, "The size must be greater than 3 or less than 20.");
					strBdSize = JOptionPane.showInputDialog (null, "Enter in the size of the board: ");
					bdSize = Integer.parseInt(strBdSize);
				}
				...
				JOptionPane.showMessageDialog(null, "You may only enter a number.");
			}
		}
	}
	...
	private int askForNumberOfPlayers() {
		...
		while (num < 2 || num > 10) {
			try {
				String strNum = JOptionPane.showInputDialog("How many players are there?");
				...
				if (num < 2 || num > 10) 
					JOptionPane.showMessageDialog(null, "The number must be between 2 and 10.");
			}
			catch (NumberFormatException e) {
				JOptionPane.showMessageDialog(null, "You may only enter a number.");
			}
	...
	private int askForStartingPlayer(int total) {
		...
		while (num < 1 || num > total) {
			try {
				String strNum = JOptionPane.showInputDialog("Which player will start?");
				...
				if (num < 1 || num > total) 
					JOptionPane.showMessageDialog(null, "The number must be between 1 and " + total + ".");
			}
			catch (NumberFormatException e){
				JOptionPane.showMessageDialog(null, "You may only enter a number.");
			}
		...
		for (int row = 0; row < bdSize; row++) {
			for (int col = 0; col < bdSize; col++) {
				...
				if (c != null && c.isLandmine() == false) {
					for (int i = 0; i <= 10; i++) {
						if (c.getNum() == i) {
							board[row][col].setIcon(new ImageIcon("number" + i + ".png"));
						}
					}
				}
				// Reveal position of exploded landmines
				else if (c != null && c.used()) {
					board[row][col].setIcon(new ImageIcon("explosion.png"));
				}
				...
				for (int col = 0; col < bdSize; col++) {
					...
					if (e.getSource() == board[row][col]) {
						...
						if (game.isLandMine(row, col)) {
							...
							// and eliminate them.
							JOptionPane.showMessageDialog(null, "Player " + game.getCurrentPlayer() + " has exploded!");
							...
							if (game.isWinner() != -1) {
								JOptionPane.showMessageDialog(null, "Player " + game.isWinner() + " has won the game!");
								turnLabel.setText("Player " + game.isWinner() + " has won!");
						...
						// a landmine
							...
							while (game.isSurrounded() != -1) {
								JOptionPane.showMessageDialog(null, "Player " + game.isSurrounded() + " has been eliminated!");
							...
							if (game.isWinner() != -1) {
								JOptionPane.showMessageDialog(null, "Player " + game.isWinner() + " has won the game!");
								turnLabel.setText("Player " + game.getCurrentPlayer() + " has won!");
								winsTracker[game.isWinner()-1]++;
								status = GameStatus.Win;
							} else if (game.cats()){
								JOptionPane.showMessageDialog(null, "The game has ended in cats, no one wins!");
